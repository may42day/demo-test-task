...
class TestDocumentArchive:
    def test_archive_document_ok(self, api_client, user):
        ...
        assert document.status == Document.ARCHIVED  
...
class TestDocumentUpdate:
    def test_update_document_ok(self, api_client, user):
        ...
        new_title = "Title#2"
        response = api_client.patch(f"/api/v1/document/{document.id}/", data={"title": new_title})
    ...
    def test_update_document_not_ok(self, api_client, user):
        ...
        response = api_client.patch(f"/api/v1/document/{document.id}/", data={"title": "Title"})
